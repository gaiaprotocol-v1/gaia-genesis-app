...
class Mining {
    ...
    async loadGenesisGaiaKlay() {
        ...
        if (this.container.deleted !== true) {
            this.genesisInterestBalanceDisplay.empty().appendText(`${CommonUtil_1.default.numberWithCommas(ethers_1.utils.formatEther(klay))} KLAY`);
            this.krnoDisplay.empty().appendText(`${CommonUtil_1.default.numberWithCommas(ethers_1.utils.formatUnits(krno, 9))} KRNO`);
        }
    }
    async loadGaiaKlay() {
        const klay = await GaiaOperationContract_1.default.claimableKlay([0]);
        if (this.container.deleted !== true) {
            const total = Number(ethers_1.utils.formatEther(klay)) * 2177;
